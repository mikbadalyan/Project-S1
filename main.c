...
/**
 * @brief This function is for counting the lines of the file and the bytes of
 * each line.
 *
 * @param buf is used for analysing the file content where all data of solar
 * panels are stored.
 * @param bytes has the value of all bytes in data file.
 * @param bytes_per_line is the number of all bytes in the current line. It gets
 * 0 value after counting the bytes of that line.
 * @param niqanak is the number of new lines in a code, if there is at least 2
 * characters.
 * @param arr_bytes is an array that stores the quantity of bytes in each line.
 * @return int The return value is 0 which indicates that the function has
 * ended.
 */
int count_of_lines(char *buf, int bytes, int bytes_per_line, int niqanak,
                   int *arr_bytes) {
...
/**
 * @brief Using this function and giving the proper parameters we get 2 most
 * profitable solar panels.
 *
 * @param root is the binary search tree which contains the data of solar
 * panels.
 * @param profitab is the array which has 2 elements. It stores the profits from
 * two most profitable two solar panels.
 * @return int It returns 0 when when all parameters are given properly and
 * function has ended.
 */
 ...
 * @param root is the binary search tree which contains the data of solar
 * panels.
 * @param most_quantity is an array which has 2 elements. It stores the
 * quantities from two solar panels which have more demand.
 * @return int It returns 0 when when all parameters are given properly and
 * function has ended.
 */
 ...
 * @param prof_power is the data from the binary search tree for the first
 * element of profitab.
 * @param prof_quantity is the data from the binary search tree for the first
 * element of profitab.
 * @param prof_size is the data from the binary search tree for the first
 * element of profitab.
 * @param prof2_power is the data from the binary search tree for the second
 * element of profitab.
 * @param prof2_quantity is the data from the binary search tree for the second
 * element of profitab.
 * @param prof2_size is the data from the binary search tree for the second
 * element of profitab.
 * @return int The function returns 0 when when all parameters are given
 * properly and function has ended.
 */
int get(struct Type_Of_Panel **root, int *profitab, char *prof_power,
        char *prof_quantity, char *prof_size, char *prof2_power,
        char *prof2_quantity, char *prof2_size) {
  if (0 != (*root)->left) {
    get(&(*root)->left, profitab, prof_power, prof_quantity, prof_size,
        prof2_power, prof2_quantity, prof2_size);
  }
  if (0 != (*root)->right) {
    get(&(*root)->right, profitab, prof_power, prof_quantity, prof_size,
        prof2_power, prof2_quantity, prof2_size);
  }
 ...
 *
 * @param root is the given binary search tree
 * @param most_quantity is the array which has 2 values of solar panel
 * quantities.
 * @param prof_power is the data from the binary search tree for the first
 * element of most_quantity.
 * @param prof_quantity is the data from the binary search tree for the first
 * element of most_quantity.
 * @param prof_size is the data from the binary search tree for the first
 * element of most_quantity.
 ...
 * properly and function has ended.
 */
int get_quantity(struct Type_Of_Panel **root, int *most_quantity,
                 char *prof_power, char *prof_quantity, char *prof_size,
                 char *prof2_power, char *prof2_quantity, char *prof2_size) {
  if (0 != (*root)->left) {
    get_quantity(&(*root)->left, most_quantity, prof_power, prof_quantity,
                 prof_size, prof2_power, prof2_quantity, prof2_size);
  }
  if (0 != (*root)->right) {
    get_quantity(&(*root)->right, most_quantity, prof_power, prof_quantity,
                 prof_size, prof2_power, prof2_quantity, prof2_size);
  }
...
/**
 * @brief This function counts all raw material which are neccessary to
 * manifacture solar panels.
 *
 * @param root is the given binary search tree
...
int main() {
  ...
  // Storing the data of the buffer in separate arrays. Each line of the file
  // is in one array.
  if (isdigit(buf[0])) {
    powe[0] = atoi(&buf[0]);
    power_coef = 0;
  ...
  printf("Profit 1 %d,,,,,,, Profit 2 %d", profitab[0], profitab[1]);
  ...
  printf("\nHigh demand 1 %d", most_quantity[0]);
  printf("\nHigh demand 2 %d", most_quantity[1]);
  // Storing all raw material data in variables.
  long int total_polysillicon;
  double total_silver;
  double total_aluminum;

  check_raw_material(&acc_to_power, &total_polysillicon, &total_silver,
                     &total_aluminum);
  check_raw_material(&acc_to_second, &total_polysillicon, &total_silver,
                     &total_aluminum);
  ...
  char prof2_size[7];
  get(&acc_to_power, profitab, prof_power, prof_quantity, prof_size,
      prof2_power, prof2_quantity, prof2_size);
  get(&acc_to_second, profitab, prof_power, prof_quantity, prof_size,
      prof2_power, prof2_quantity, prof2_size);
  ...
  get_quantity(&acc_to_power, most_quantity, prof_power, prof_quantity,
               prof_size, prof2_power, prof2_quantity, prof2_size);
  get_quantity(&acc_to_second, most_quantity, prof_power, prof_quantity,
               prof_size, prof2_power, prof2_quantity, prof2_size);
